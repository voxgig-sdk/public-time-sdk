...
boru-driven command-line client **and** interactive REPL for the PublicTime
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/public-time-cli)
make build

# 2. See usage (words, entities, env vars)
./public-time-cli --help

# 3. Provide credentials once, via the environment
export PUBLIC_TIME_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./public-time-cli load 1 time            # {id:1} shorthand
./public-time-cli load '{id:1}' time       # explicit match map

# 5. Override the API base URL for a single call
PUBLIC_TIME_BASE=https://api.example.com ./public-time-cli load 1 time

# 6. No arguments -> interactive REPL
./public-time-cli
public-time> load 1 time
public-time> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/public-time-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/public-time-cli load 1 time
   ```
...
```sh
./public-time-cli load 1 time          # scalar shorthand for {id:1}
./public-time-cli load '{id:1}' time     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `public-time>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `time`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/public-time-cli`. |
...
(`./public-time-cli <expr>`) and interactively (the REPL), and expressions compose the
