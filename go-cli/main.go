// Command PublicTime-cli is a CLI + REPL for the PublicTime SDK,
...
const prompt = "public-time"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "time timestamp"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      PUBLIC_TIME_APIKEY (api key), PUBLIC_TIME_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: PUBLIC_TIME_APIKEY carries the API key and
	// PUBLIC_TIME_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("PUBLIC_TIME_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("PUBLIC_TIME_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewPublicTimeSDK(opts)
