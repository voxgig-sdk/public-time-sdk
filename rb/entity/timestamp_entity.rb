...
class TimestampEntity
  ...
  def make
    opts = @_entopts.dup
    TimestampEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = PublicTimeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Timestamp, Hash] the current Timestamp data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = PublicTimeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Timestamp fields)
  def match_get
  ...
  #
  # @param reqmatch [TimestampLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Timestamp.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Timestamp, Hash] the loaded Timestamp; raises PublicTimeError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = PublicTimeHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
