...
class TimeEntity
  ...
  # @return [Time, Hash] the current Time data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Time fields)
  def match_get
  ...
  # Load a single Time.
  #
  # @param reqmatch [TimeLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Time.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Time, Hash] the loaded Time; raises PublicTimeError on failure
