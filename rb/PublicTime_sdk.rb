...
require_relative 'PublicTime_types'
...
class PublicTimeSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = PublicTimeHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = PublicTimeHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, PublicTimeFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    PublicTimeUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = PublicTimeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = PublicTimeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = PublicTimeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = PublicTimeHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = PublicTimeError.new(
        "graphql_error", "PublicTimeSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.Time.list / client.Time.load({ "id" => ... })
  def Time(data = nil)
    require_relative 'entity/time_entity'
    TimeEntity.new(self, data)
  end


  # Canonical facade: client.Timestamp.list / client.Timestamp.load({ "id" => ... })
  def Timestamp(data = nil)
    require_relative 'entity/timestamp_entity'
    TimestampEntity.new(self, data)
  end
