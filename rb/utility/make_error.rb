...
module PublicTimeUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = PublicTimeContext.new({}, nil)
    end
    op = ctx.op || PublicTimeOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || PublicTimeResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(PublicTimeError) ? err.msg : err.to_s
    ...
    sdk_err = PublicTimeError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(PublicTimeError)
