...
module PublicTimeUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = PublicTimeResponse.new({})
    ...
    if fetch_err
      ...
      response = PublicTimeResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PublicTimeResponse.new(fetched)
    else
