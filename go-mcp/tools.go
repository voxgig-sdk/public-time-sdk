...
type Args struct {
	Entity string         `json:"entity" jsonschema:"time | timestamp"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.PublicTimeSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "public-time_list",
		Description: "List records from PublicTime. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "public-time_load",
		Description: "Load a single record from PublicTime. " +
...
func runOp(client *sdk.PublicTimeSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.PublicTimeSDK, name string) (sdk.PublicTimeEntity, error) {
	switch strings.ToLower(name) {
	case "time":
		return client.Time(nil), nil
	case "timestamp":
		return client.Timestamp(nil), nil
