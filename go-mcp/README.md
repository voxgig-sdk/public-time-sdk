...
[MCP](https://modelcontextprotocol.io) server exposing the PublicTime SDK as
two agent tools — `public-time_list` and `public-time_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/public-time-mcp)
make build

# 2. Provide credentials via the environment
export PUBLIC_TIME_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user public-time \
  -- /absolute/path/to/public-time-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./public-time-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// public-time_list: first page of records
{ "entity": "time" }
{ "entity": "time", "query": { } }

// public-time_load: one record by id
{ "entity": "time", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/public-time-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user public-time \
     -- "$PWD"/dist/*/public-time-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `public-time_list` and `public-time_load` tools now appear
   in new sessions. Ask the agent to *"list time using public-time"*
   and it calls `public-time_list` with `{"entity":"time"}`.
...
```sh
export PUBLIC_TIME_APIKEY=sk_live_xxx            # API key
export PUBLIC_TIME_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `public-time_list` tool
...
### Call the `public-time_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `public-time_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `public-time_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PUBLIC_TIME_APIKEY` | API key sent with every request. |
| `PUBLIC_TIME_BASE` | Optional override of the API base URL. |
...
```sh
./public-time-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"public-time_load","arguments":{"entity":"time","query":{"id":1}}}}'
```
