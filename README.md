...
Public Time API client, generated from the OpenAPI spec.

This API provides the current timestamp in various formats, including text, JSON, and CSS custom properties.

Learn more about Public Time API at [zenn.dev/siyukatu/articles/public-time-api?locale=en](https://zenn.dev/siyukatu/articles/public-time-api?locale=en).

This is an unofficial SDK for the Public Time public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — Time and Timestamp — that you
...
```go
client := sdk.Test()
result, err := client.Time(nil).Load(
    nil, nil,
)
```
...
| --- | --- | --- |
| TypeScript | `@voxgig-sdk/public-time` | publish pending — [install from git tag](https://github.com/voxgig-sdk/public-time-sdk/releases) |
...
```bash
cd go-mcp && go build -o public-time-mcp .
```
...
```json
{
  "mcpServers": {
    "public-time": {
      "command": "/abs/path/to/public-time-mcp"
    }
  }
}
```
...
| --- | --- | --- |
| **Time** | The Time entity (load). | `/time.json` |
| **Timestamp** | The Timestamp entity (load). | `/time/events` |
...
# Load a specific time (returns the record, raises on error)
time = client.Time().load()
print(time)
```
...
```go
import sdk "github.com/voxgig-sdk/public-time-sdk/go"
...
Generated from the Public Time API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
