...
// PublicTime SDK Timestamp entity
...
class TimestampEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new TimestampEntity($this->_client, $opts);
    }

    /**
     * @param Timestamp|array $args Timestamp data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = PublicTimeHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Timestamp|array The current Timestamp data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Timestamp fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = PublicTimeHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Timestamp fields).
     */
     ...
     *
     * @param TimestampLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed TimestampLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Timestamp|array The loaded Timestamp as an assoc-array at the
     *   SDK boundary; throws PublicTimeError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = PublicTimeHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
