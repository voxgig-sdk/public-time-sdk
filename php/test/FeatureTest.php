...
class FtCtrl extends PublicTimeControl
{
...
class FtClient
{
    ...
    private PublicTimeUtility $utility;

    public function __construct(PublicTimeUtility $utility, array $options)
    {
    ...
    public function get_utility(): PublicTimeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PublicTimeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PublicTimeContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PublicTimeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PublicTimeConfig::make_feature($name);
    ...
    private static function buildUrl(PublicTimeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PublicTimeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PublicTimeResponse($fetched);
        }
    ...
    private function fail(PublicTimeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PublicTimeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
