...
// PublicTime SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class PublicTimeMakeError
{
    public static function call(?PublicTimeContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new PublicTimeContext([], null);
        }
        $op = $ctx->op ?? new PublicTimeOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new PublicTimeResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof PublicTimeError) ? $err->msg : (string)$err;
        $msg = "PublicTimeSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new PublicTimeError('', $msg, $ctx);
        ...
        if ($err instanceof PublicTimeError) {
