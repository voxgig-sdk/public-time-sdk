...
// PublicTime SDK primary utility test
//
...
//   PublicTimeOmni::unwrap, which throws the err so omni can match it
...
//   builds the typed context with PublicTimeOmni::ctx_from_map, runs the
//   utility, and writes the observable state back with
//   PublicTimeOmni::sync_ctx BEFORE throwing any error - that map is
...
class PrimaryUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = PublicTimeOmni::makeRunner(
                self::TEST_JSON_FILE, PublicTimeSDK::test(null, null));
    ...
    private static function client(): PublicTimeSDK
    {
    ...
    private static function make_test_ctx(
        PublicTimeSDK $client,
        PublicTimeUtility $utility,
    ...
    {
        $client = PublicTimeSDK::test(null, null);
    ...
    public function test_done_basic(): void
    {
        ...
        $this->runsection('done', function (array $ctxmap) use ($client, $utility) {
            $ctx = PublicTimeOmni::ctx_from_map($ctxmap, $client, $utility);
    ...
    public function test_make_error_basic(): void
    {
        ...
        $this->runsection('makeError', function ($ctxmap = null, $errmap = null) use ($client, $utility) {
            $ctx = PublicTimeOmni::ctx_from_map(
                is_array($ctxmap) ? $ctxmap : [], $client, $utility);
            $err = PublicTimeOmni::err_from_map(is_array($errmap) ? $errmap : null);
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new PublicTimeResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new PublicTimeBaseFeature();
    ...
    {
        $hook_client = PublicTimeSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends PublicTimeBaseFeature {
            public $hook_fn = null;
            public function TestHook(PublicTimeContext $ctx): void
            {
    ...
    {
        $init_client = PublicTimeSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends PublicTimeBaseFeature {
            public $init_fn = null;
    ...
    {
        $blocked_client = new PublicTimeSDK([
            'base' => 'http://localhost:8080',
            'system' => [
    ...
    public function test_make_request_basic(): void
    {
        ...
        $this->runsection('makeRequest', function (array $ctxmap) use ($client, $utility) {
            ...
            PublicTimeOmni::sync_ctx($ctxmap, $ctx);
            if ($err !== null) {
                throw $err;
            }
            return null;
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new PublicTimeSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = PublicTimeSDK::test(null, is_array($setup) ? $setup : null);
        $spec_utility = $spec_client->get_utility();
        $utility = self::client()->get_utility();

        $this->runsection('makeSpec', function (array $ctxmap) use ($spec_client, $spec_utility, $utility) {
            $ctx = PublicTimeOmni::ctx_from_map($ctxmap, $spec_client, $spec_utility);
    ...
    public function test_make_url_basic(): void
    {
        ...
        $this->runsection('makeUrl', function (array $ctxmap) use ($client, $utility) {
            ...
            return PublicTimeOmni::unwrap(($utility->make_url)($ctx));
        });
    }
    ...
    public function test_operator_basic(): void
    {
        $this->runsection('operator', function ($vin = null) {
            ...
            $op = new PublicTimeOperation($vin);
            return [
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = PublicTimeSDK::test(null, is_array($setup) ? $setup : null);
        ...
        $this->runsection('prepareAuth', function (array $ctxmap) use ($auth_client, $auth_utility, $utility) {
            $ctx = PublicTimeOmni::ctx_from_map($ctxmap, $auth_client, $auth_utility);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runsection('resultBasic', function (array $ctxmap) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof PublicTimeError)
