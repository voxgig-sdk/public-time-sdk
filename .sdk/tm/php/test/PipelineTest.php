...
{
    private static function utility(): PublicTimeUtility
    {
        return new PublicTimeUtility();
    }
    ...
    private static function ctx(array $over = []): PublicTimeContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new PublicTimeContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new PublicTimeOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof PublicTimeError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new PublicTimeOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = PublicTimeMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new PublicTimeOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = PublicTimeMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new PublicTimeOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = PublicTimeMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PublicTimeSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = PublicTimeMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new PublicTimeResponse([]),
            'result' => new PublicTimeResult([]),
        ]);
        [, $err] = PublicTimeMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new PublicTimeSpec([]),
            'response' => null,
            'result' => new PublicTimeResult([]),
        ]);
        [, $err] = PublicTimeMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new PublicTimeSpec(['step' => 's']),
            'response' => new PublicTimeResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new PublicTimeResult([]),
        ]);
        [, $err] = PublicTimeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new PublicTimeSpec(['step' => 's']),
            'response' => new PublicTimeResponse(self::resp(200, ['v' => 1])),
            'result' => new PublicTimeResult([]),
        ]);
        [, $err] = PublicTimeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new PublicTimeSpec(['step' => 's']),
            'response' => new PublicTimeResponse(self::resp(200, ['v' => 2])),
            'result' => new PublicTimeResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        PublicTimeMakeResponse::call($ctx);
    ...
    {
        $preset = new PublicTimeResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new PublicTimeSpec([]),
            'response' => new PublicTimeResponse([]),
            'result' => new PublicTimeResult([]),
        ...
        [$got, $err] = PublicTimeMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new PublicTimeResult([])]);
        [, $err] = PublicTimeMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new PublicTimeSpec([]), 'result' => null]);
        [, $err] = PublicTimeMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PublicTimeSpec(['step' => 's']),
            'result' => new PublicTimeResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new PublicTimeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PublicTimeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PublicTimeSpec(['step' => 's']),
            'result' => new PublicTimeResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new PublicTimeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PublicTimeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PublicTimeResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new PublicTimeSpec([]),
            'result' => new PublicTimeResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = PublicTimeMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PublicTimeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new PublicTimeError('boom', 'boom');
        $utility->fetcher = function (PublicTimeContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PublicTimeSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = PublicTimeMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (PublicTimeContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = PublicTimeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(PublicTimeResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        PublicTimeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (PublicTimeContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PublicTimeSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = PublicTimeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new PublicTimeResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new PublicTimeSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = PublicTimeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = PublicTimeMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = PublicTimeMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new PublicTimeResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, PublicTimeDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new PublicTimeResult(['ok' => false])]);
        $this->expectException(PublicTimeError::class);
        PublicTimeDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new PublicTimeResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', PublicTimeMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        PublicTimeMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new PublicTimeBaseFeature();
        $b = new PublicTimeBaseFeature();
        PublicTimeFeatureAdd::call($ctx, $a);
        PublicTimeFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): PublicTimeBaseFeature
    {
        $f = new PublicTimeBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        PublicTimeFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): PublicTimeContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new PublicTimeSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PublicTimePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        PublicTimePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new PublicTimeResponse(['headers' => null]),
            'result' => new PublicTimeResult([]),
        ]);
        PublicTimeResultHeaders::call($ctx);
