...
from publictime_sdk.utility import register

# Load features
from publictime_sdk.feature.base_feature import PublicTimeBaseFeature
from publictime_sdk.features import _has_feature, _make_feature
...
class PublicTimeSDK:
    ...
    def get_utility(self):
        return PublicTimeUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("PublicTimeSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def Time(self, data=None) -> "TimeEntity":
        """Entity factory: client.Time().list() / client.Time().load({"id": ...})."""
        from publictime_sdk.entity.time_entity import TimeEntity
        return TimeEntity(self, data)


    def Timestamp(self, data=None) -> "TimestampEntity":
        """Entity factory: client.Timestamp().list() / client.Timestamp().load({"id": ...})."""
        from publictime_sdk.entity.timestamp_entity import TimestampEntity
        return TimestampEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "PublicTimeSDK":
        if sdkopts is None:
            sdkopts = {}
