...
from publictime_sdk.publictime_types import (
    Timestamp,
    TimestampLoadMatch,
)
...
class TimestampEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return TimestampEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Timestamp:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Timestamp().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
