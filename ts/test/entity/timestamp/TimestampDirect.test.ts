...
describe('TimestampDirect', async () => {
  ...
  test('direct-load-timestamp', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-timestamp', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['PUBLIC_TIME_TEST_TIMESTAMP_ENTID']
