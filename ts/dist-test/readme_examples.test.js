...
const TEST_SEED = { "entity": { "time": { "test01": { "id": "test01" } }, "timestamp": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+PublicTimeSDK\b/.test(code) ||
        /\bPublicTimeSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+PublicTimeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/PublicTimeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
