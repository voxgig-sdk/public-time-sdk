...
import (
	...
	omni "github.com/voxgig-sdk/public-time-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PublicTimeError); is {
				out := map[string]any{
					"name":    "PublicTimeError",
...
func omniCtx(arg any, client *sdk.PublicTimeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PublicTimeSDK); is {
			client = live
