...
local PublicTimeSDK = {}
PublicTimeSDK.__index = PublicTimeSDK
...
PublicTimeSDK._make_feature = _make_feature


function PublicTimeSDK.new(options)
  local self = setmetatable({}, PublicTimeSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function PublicTimeSDK:options_map()
...
function PublicTimeSDK:get_utility()
  return Utility.copy(self._utility)
end


function PublicTimeSDK:get_root_ctx()
  return self._rootctx
end


function PublicTimeSDK:prepare(fetchargs)
...
function PublicTimeSDK:direct(fetchargs)
...
-- by passing it.
function PublicTimeSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function PublicTimeSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "PublicTimeSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:Time():list() / client:Time():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function PublicTimeSDK:Time(data)
  local EntityMod = require("entity.time_entity")
  if data == nil then
    if self._time == nil then
      self._time = EntityMod.new(self, nil)
    end
    return self._time
  end
...
-- Idiomatic facade: client:Timestamp():list() / client:Timestamp():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function PublicTimeSDK:Timestamp(data)
  local EntityMod = require("entity.timestamp_entity")
  if data == nil then
    if self._timestamp == nil then
      self._timestamp = EntityMod.new(self, nil)
    end
...
function PublicTimeSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = PublicTimeSDK.new(sdkopts)
  sdk.mode = "test"
