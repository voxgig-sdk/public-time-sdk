...
local TimestampEntity = {}
TimestampEntity.__index = TimestampEntity


function TimestampEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TimestampEntity)
...
function TimestampEntity:get_name()
  return self._name
end


function TimestampEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TimestampEntity.new(self._client, opts)
end
...
function TimestampEntity:mark_deleted()
  self._deleted = true
end


function TimestampEntity:deleted()
  return true == self._deleted
end


function TimestampEntity:data_set(args)
  if args ~= nil then
...
function TimestampEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TimestampEntity:match_set(args)
  if args ~= nil then
...
function TimestampEntity:match_get()
...
function TimestampEntity:stream(action, args, callopts)
...
---@param reqmatch TimestampLoadMatch
---@param ctrl? table
---@return Timestamp
---@return string? err
function TimestampEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function TimestampEntity:_run_op(ctx, post_done)
  local utility = self._utility
