...
function TimeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TimeEntity)
  self._name = "time"
  self._client = client
...
function TimeEntity:get_name()
  return self._name
end


function TimeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TimeEntity.new(self._client, opts)
end
...
function TimeEntity:mark_deleted()
  self._deleted = true
end


function TimeEntity:deleted()
  return true == self._deleted
end


function TimeEntity:data_set(args)
  if args ~= nil then
...
function TimeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TimeEntity:match_set(args)
  if args ~= nil then
...
function TimeEntity:stream(action, args, callopts)
...
---@param reqmatch TimeLoadMatch
---@param ctrl? table
---@return Time
---@return string? err
function TimeEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function TimeEntity:_run_op(ctx, post_done)
  local utility = self._utility
